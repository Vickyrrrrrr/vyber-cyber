...
def run_duel_stream(scenario_id: int) -> Generator[tuple[str, str, str], None, None]:
    ...
    red_terminal = "SYSTEM: Sandbox environment created. Target files deployed.\n"
    blue_terminal = "SYSTEM: Monitoring channel active. SOC Agent waiting for anomaly triggers.\n"
    ...
    if use_simulation:
        if scenario_id == 1:
            ...
            red_terminal += "\nRED_TEAM_AGENT: Initializing Attack Reconnaissance...\n"
            red_terminal += "RED_TEAM_AGENT: Thought - Locate configuration files to extract local parameters.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to list directory.\n"
            ...
            red_terminal += "RED_TEAM_AGENT: Thought - app_config.json located. Reading file content.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to read app_config.json.\n"
            yield (red_terminal, blue_terminal, "Attack Reconnaissance phase active...")
            time.sleep(2.0)
            ...
            red_terminal += "RED_TEAM_AGENT: Exposed plaintext credentials identified.\n"
            red_terminal += "RED_TEAM_AGENT: Formulating exploit plan...\n"
            exploit_plan = {
            ...
            red_terminal += f"RED_TEAM_AGENT: Action - Outputting JSON exploit plan:\n{json.dumps(exploit_plan, indent=4)}\n"
            yield (red_terminal, blue_terminal, "Target compromised. Exploit verified.")
            time.sleep(3.0)
            
            # --- Scenario 1: Blue Team Detection ---
            blue_terminal += "\nBLUE_TEAM_AGENT: ALERT - Unauthorized credential access detected in sandbox config.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Thought - app_config.json contains plaintext credentials. Rewriting with environment variables.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - Use edit tool to rewrite app_config.json.\n"
            ...
            blue_terminal += f"\n[EDITED /tmp/sandbox/app_config.json]:\n{json.dumps(healed_config, indent=4)}\n\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.5)
            
            blue_terminal += "BLUE_TEAM_AGENT: Thought - Restricting configuration file permissions.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - OpenCode shell: chmod 600 app_config.json\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.0)
            
            subprocess.run("chmod 600 app_config.json", shell=True, cwd=base_dir)
            blue_terminal += "BLUE_TEAM_AGENT: File permissions locked down. Initiating validation verification scan.\n"
            ...
            red_terminal += "\nRED_TEAM_AGENT: Initializing Attack Reconnaissance...\n"
            red_terminal += "RED_TEAM_AGENT: Thought - Scan database parameters for exposed ports.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to list configuration files.\n"
            yield (red_terminal, blue_terminal, "Attack Reconnaissance phase active...")
            time.sleep(2.0)
            ...
            red_terminal += "RED_TEAM_AGENT: Thought - db_settings.yaml identified. Reading configuration parameters.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to read db_settings.yaml.\n"
            yield (red_terminal, blue_terminal, "Attack Reconnaissance phase active...")
            time.sleep(2.0)
            ...
            red_terminal += "RED_TEAM_AGENT: Vulnerability Identified - Database bound globally with auth disabled.\n"
            red_terminal += "RED_TEAM_AGENT: Formulating exploit plan...\n"
            exploit_plan = {
            ...
            blue_terminal += "\nBLUE_TEAM_AGENT: ALERT - Unauthenticated global port binding active on port 5432.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Thought - Restrict database binding to localhost (127.0.0.1) and enforce authentication.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - Use edit tool to update db_settings.yaml.\n"
            yield (red_terminal, blue_terminal, "Anomaly detected. Initializing Automated SOC Agent...")
            time.sleep(3.0)
            ...
            blue_terminal += f"\n[EDITED /tmp/sandbox/db_settings.yaml]:\n{yaml.safe_dump(healed_db_config, default_flow_style=False)}\n\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.5)
            
            blue_terminal += "BLUE_TEAM_AGENT: Local binding configured and authentication enabled. Reconfiguring firewall rules...\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - OpenCode shell: iptables blocking policy enforced.\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.0)
            
            blue_terminal += "BLUE_TEAM_AGENT: Firewall rules active. Initiating validation verification scan.\n"
            yield (red_terminal, blue_terminal, "Mitigation complete. Initiating validation verification scan...")
            time.sleep(2.0)
            ...
            red_terminal += "\nRED_TEAM_AGENT: Initializing Attack Reconnaissance...\n"
            red_terminal += "RED_TEAM_AGENT: Thought - Scan transmission configuration and active network logs.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to read logs.\n"
            yield (red_terminal, blue_terminal, "Attack Reconnaissance phase active...")
            time.sleep(2.0)
            ...
            red_terminal += "RED_TEAM_AGENT: Thought - Network logs reveal plaintext data transmissions. Inspecting pipeline configuration.\n"
            red_terminal += "RED_TEAM_AGENT: Action - Run OpenCode command to read pipeline_config.json.\n"
            yield (red_terminal, blue_terminal, "Attack Reconnaissance phase active...")
            time.sleep(2.0)
            ...
            red_terminal += "RED_TEAM_AGENT: Vulnerability Identified - Communications pipeline is unencrypted.\n"
            red_terminal += "RED_TEAM_AGENT: Formulating exploit plan...\n"
            exploit_plan = {
            ...
            blue_terminal += "\nBLUE_TEAM_AGENT: ALERT - Plaintext data transmissions identified on billing pipeline.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Thought - Enforce SSL, enable TLS verification, update target endpoint to HTTPS, and select AES-256-GCM cipher.\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - Use edit tool to update pipeline_config.json.\n"
            yield (red_terminal, blue_terminal, "Anomaly detected. Initializing Automated SOC Agent...")
            time.sleep(3.0)
            ...
            blue_terminal += f"\n[EDITED /tmp/sandbox/pipeline_config.json]:\n{json.dumps(healed_pipeline, indent=4)}\n\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.5)
            
            blue_terminal += "BLUE_TEAM_AGENT: Enforcing SSL/TLS policies and verifying CA certificates...\n"
            blue_terminal += "BLUE_TEAM_AGENT: Action - OpenCode shell: update CA certificate stores.\n"
            yield (red_terminal, blue_terminal, "Deploying self-healing code fix...")
            time.sleep(2.0)
            
            blue_terminal += "BLUE_TEAM_AGENT: CA certificates updated. Enforcing secure endpoint. Initiating validation verification scan.\n"
            yield (red_terminal, blue_terminal, "Mitigation complete. Initiating validation verification scan...")
            time.sleep(2.0)
    ...
    red_terminal += "\nRED_TEAM_AGENT: Re-running exploit scan...\n"
    if scenario_id == 1:
        out = opencode_run("Read app_config.json", base_dir)
    elif scenario_id == 2:
        out = opencode_run("Read db_settings.yaml", base_dir)
    ...
    red_terminal += f"\n{out}\n"
    if is_secure:
        red_terminal += "RED_TEAM_AGENT: EXPLOIT FAILED - Target Secured\n"
        blue_terminal += f"\nSYSTEM: {status_msg}\n"
        yield (red_terminal, blue_terminal, "Secure: Vulnerability successfully mitigated and verified.")
    else:
        red_terminal += f"RED_TEAM_AGENT: EXPLOIT SUCCESS - Vulnerability still open: {status_msg}\n"
